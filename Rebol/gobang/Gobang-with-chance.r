...
view layout [
	...
	style qbox box 30x30 238.180.34 [ 
		if face/image = none  [
			if myturn [
				face/image: heizi
				poke qp face/data to-tuple reduce [0 1 index 0]
				poke trait face/data to-tuple reduce [0 1 0 0]
				show face 
				v: val face/data myturn
				v: v/1
				if (v >= 5) [
					alert "Black Win"
					qp/1: qp/1 + 0.0.0.1
					fname: rejoin reduce [ %./qp/ form now/date to-integer now/time ".png"]
					save/png fname qp 
					wait 1
					for i 1 225 1 [
						do rejoin reduce [ "C" i {/image: none}]
						do rejoin reduce [ "C" i {/text: none}]
			...
			if (not myturn) [
				choice: chance-alg myturn
				click choice/5 myturn
				v: val choice/5 myturn
				v: v/1
				...
				t1/text: " Your turn "
				t2/text: to-string choice/2
				either (choice/2 = 0) [
					t3/text: "None"
				] [
					t3/text: rejoin reduce [ to-string choice/1 "%"]
				]
				show t1
				show t2
				show t3
	...
	panel 466x503 black [
		...
		space 1
		below
		panel 466x35 240.255.255 [
			origin 5x5
			space 5x5
			across
			pad 20x0
			t1: text " Your turn " with [ size: 120x30 font/size: 15 ]
			text "Number Of Matched:" with [ size: 150x30]
			t2: text "" with [ size: 40x30]
			text " Chance:" with [ size: 60x30]
			t3: text "" with [ size: 50x30]
		]
		panel [
			origin 1
			space 1
